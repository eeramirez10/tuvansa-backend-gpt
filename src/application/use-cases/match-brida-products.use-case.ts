...
import { ExtractBridaPropertiesUseCase } from "./extract-brida-properties.use-case";
import { MatchProductUseCase } from "./match-product.use-case";

export class MatchBridaProductsUseCase {
  ...
  async execute(description: string) {
    const filters = await new ExtractBridaPropertiesUseCase(this.languageModelService).execute(description)
